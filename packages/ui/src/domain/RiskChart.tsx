  ...
  return (
    <span>
      <svg viewBox="0 0 16 16" aria-hidden="true">
        {kind === 'decision' ? (
          <path className={styles.chartDecision} d="M 8 2 L 14 8 L 8 14 L 2 8 Z" />
        ) : kind === 'after' ? (
          <path className={styles.chartAfterLine} d="M 1 8 H 15" />
        ) : (
...
export function RiskChart({
  ...
  const span = Date.parse(last.startedAt) - start;
  ...
  const points = ordered.map((visit, index) => ({
    ...
    beforeY: scoreY(visit.scoreBefore),
  }));
  const decisionIndex = points.findIndex(({ visit }) => visit.id === blockedAtVisitId);
  const decision = points[decisionIndex];
  const selected =
    points.find(({ visit }) => visit.id === (selectedVisitId ?? localSelectedId)) ??
    decision ??
    points[points.length - 1];
  const inspected = points.find(({ visit }) => visit.id === (hoveredId ?? focusedId)) ?? selected;
  const path = (startIndex: number, endIndex: number, continuation = false) =>
    riskStepPath(points.slice(startIndex, endIndex), continuation);
  const beforePath = path(0, decisionIndex >= 0 ? decisionIndex + 1 : points.length);
  const afterPath =
    decisionIndex >= 0 && decisionIndex < points.length - 1 ? path(decisionIndex, points.length, true) : '';
    ...
    points
      .filter((point, index) => {
        if (
          ...
          point === decision ||
          Math.hypot(point.x - previousMarker.x, point.y - previousMarker.y) >= 10
        ) {
  ...
  return (
    <div className={`${styles.riskChart} ${styles[`chart-${size}`]}`}>
      <div className={styles.chartHeading}>
        <span>Risk score</span>
        <span className={styles.chartThresholdKey}>Block threshold · {threshold}</span>
      </div>
      <svg
        className={styles.chartPlot}
        viewBox={`0 0 ${width} ${height}`}
        role="group"
        aria-label={`Risk journey. Block threshold ${threshold}.`}
        aria-describedby={descriptionId}
        onMouseLeave={() => setHoveredId(undefined)}
      >
        <desc id={descriptionId}>
          Risk score from 0 to 100 over time. Markers show recorded times and scores; steps connect visits,
          not measurements between them. Arrow keys move between visits; Enter or Space selects a visit.
          Diamond marks the block decision, not its delivery status.
        </desc>
        <rect
          className={styles.chartRiskZone}
          x={left}
          y={top}
          width={right - left}
          height={thresholdY - top}
        />
        {ticks.map((score) => (
        ...
        {points.map((point) => {
          ...
          return (
            <g
              key={point.visit.id}
              ref={(element) => {
                if (element) markerRefs.current.set(point.visit.id, element);
                else markerRefs.current.delete(point.visit.id);
              }}
              className={`${styles.chartPoint} ${isSelected ? styles.chartSelected : ''}`}
              role="button"
              tabIndex={point.visit.id === (focusedId ?? selected.visit.id) ? 0 : -1}
              aria-label={`Visit ${point.index + 1}, score ${point.visit.scoreAfter}`}
              aria-pressed={isSelected}
              onMouseEnter={() => setHoveredId(point.visit.id)}
              onFocus={() => setFocusedId(point.visit.id)}
              onBlur={() => setFocusedId(undefined)}
              onClick={() => choose(point.visit.id)}
              onKeyDown={(event) => handleKey(event, point.index)}
            >
              <circle className={styles.chartHitArea} cx={point.x} cy={point.y} r="9" />
              {shown &&
                (point === decision ? (
                  <path
                    className={styles.chartDecision}
                    d={`M ${point.x} ${point.y - 5} L ${point.x + 5} ${point.y} L ${point.x} ${point.y + 5} L ${point.x - 5} ${point.y} Z`}
                  />
                ) : (
                  <circle
                    className={point.visit.source === 'paid' ? styles.chartPaid : styles.chartUnpaid}
                    cx={point.x}
                    cy={point.y}
                    r="4"
                  />
        ...
        <g aria-hidden="true" className={styles.chartAxisLabel}>
          <text x={left} y={bottom + 18}>
            {dateLabel(first.startedAt)}
          </text>
          <text x={left} y={bottom + 33}>
            {timeLabel(first.startedAt)}
          </text>
          <text x={(left + right) / 2} y={bottom + 26} textAnchor="middle">
            {points.length === 1 ? '1 visit' : 'Time →'}
          </text>
          {points.length > 1 && (
            <>
              <text x={right} y={bottom + 18} textAnchor="end">
                {dateLabel(last.startedAt)}
              </text>
              <text x={right} y={bottom + 33} textAnchor="end">
                {timeLabel(last.startedAt)}
              </text>
            </>
          )}
        </g>
      ...
      <div className={styles.chartLegend}>
        <ChartKey kind="paid" label="Paid" />
        <ChartKey kind="unpaid" label="Unpaid" />
        {decision && <ChartKey kind="decision" label="Decision" />}
        {afterPath && <ChartKey kind="after" label="After decision" />}
      </div>
      <div className={styles.chartReadout}>
        <div>
          <strong>
            Visit {inspected.index + 1} <span>· {timeLabel(current.startedAt)}</span>
          </strong>
          <b>
            {current.scoreBefore} <span>→</span> {current.scoreAfter}
          </b>
        </div>
        <div>
          <span>
            {current.source === 'paid'
              ? `Paid${current.platform ? ` · ${platformNames[current.platform]}` : ''}`
              : current.source[0].toUpperCase() + current.source.slice(1)}{' '}
            · {dateLabel(current.startedAt)}
          </span>
          <span className={inspected === decision ? styles.chartDecisionText : ''}>
            {inspected === decision
              ? 'Block decision'
              : decision && inspected.index > decisionIndex
                ? 'After decision'
                : points.length === 1
                  ? 'Only visit so far'
                  : `${ordered.length} visits total`}
          </span>
        </div>
      </div>
    </div>
