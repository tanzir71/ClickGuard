...
export function ExclusionList({ rows, onRetry }: { rows: ExclusionVM[]; onRetry?: () => void }) {
  if (!rows.length) return <p className={styles.muted}>Not on any exclusion list.</p>;
  return <div className={styles.exclusionList}>{rows.map((row) => <div key={row.platform} className={styles.exclusionRow}><span className={styles.platformGlyph}>{platformShort[row.platform]}</span><span><strong>{platformLabel[row.platform]}</strong><small>{row.error ?? (row.state === 'not_connected' ? 'Connect to protect future paid visits' : 'All active campaigns')}</small></span><span className={styles[`exclusion-${row.state}`]}>{row.state === 'excluded' ? '✓ Excluded' : row.state === 'syncing' ? '◌ Syncing' : row.state === 'failed' ? '! Failed' : row.state === 'removed' ? '↺ Removed' : '— Not connected'}<small>{row.at ? new Date(row.at).toLocaleString([], { month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' }) : ''}</small>{row.state === 'failed' && <Button variant="link" size="sm" onClick={onRetry}>Retry</Button>}</span></div>)}</div>;
}
