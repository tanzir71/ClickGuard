...
export function buildVisitJourney(visits: VisitVM[], threshold = 70, blockedAtVisitId?: string) {
  ...
  const y = (score: number) =>
    Number((bottom - (Math.max(0, Math.min(100, score)) / 100) * (bottom - top)).toFixed(2));
  const points = ordered.map((visit, index) => ({
    visit,
    index,
    x: Number(
      (span > 0
        ? left + ((Date.parse(visit.startedAt) - first) / span) * (right - left)
        : (left + right) / 2
      ).toFixed(2),
    ),
    y: y(visit.scoreAfter),
    beforeY: y(visit.scoreBefore),
    kind:
      visit.id === blockedAtVisitId
        ? 'decision'
        : visit.conversion
          ? 'conversion'
          : visit.source === 'paid'
  ...
  const path = (start: number, end: number, continuation = false) =>
    riskStepPath(points.slice(start, end), continuation);
  ...
  return {
    ...
    afterPath:
      triggerIndex >= 0 && triggerIndex < points.length - 1 ? path(triggerIndex, points.length, true) : '',
