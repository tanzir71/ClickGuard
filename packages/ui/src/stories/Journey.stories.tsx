...
const visitor: VisitorVM = {
  ...
  firstSeen: visits[0].startedAt, lastSeen: visits.at(-1)!.startedAt, status: 'blocked', reviewed: false, riskScore: visits.at(-1)!.scoreAfter, threshold: 70,
...
export const ConversionWhileMonitoring: Story = { args: { visitor: { ...visitor, status: 'monitoring', blockedAtVisitId: undefined, blockedAt: undefined, riskScore: 27, exclusions: [], conversions: 1, protectedSpendEst: 0, visits: visits.slice(0, 4).map((visit, index) => ({ ...visit, afterBlock: false, scoreBefore: [0, 22, 28, 34][index], scoreAfter: [22, 28, 34, 27][index], scoreSteps: index === 3 ? [{ signalId: 'bot_probability', label: 'Likely automated', points: 18 }, { signalId: 'converted', label: 'Converted', points: -25 }] : visit.scoreSteps, conversion: index === 3 ? { type: 'purchase', value: 84 } : undefined, interaction: { level: 'high', scrollPct: 78, clicks: 5, pointerMoves: 48 }, events: [{ t: 0, kind: 'landed', label: 'Landed on /sale/trail-runners' }, { t: 200, kind: 'click', label: 'Clicked View product' }, ...(index === 3 ? [{ t: 900, kind: 'conversion', label: 'Completed purchase' }] : []), { t: 1100, kind: 'exit', label: 'Session ended' }] })) } } };
export const MissingBehavior: Story = { args: { visitor: { ...visitor, status: 'monitoring', blockedAtVisitId: undefined, blockedAt: undefined, riskScore: 8, exclusions: [], wastedSpend: 4.1, protectedSpendEst: 0, visits: [{ ...visits[0], jsExecuted: false, events: [], scoreAfter: 8, scoreSteps: [{ signalId: 'no_js', label: 'No JavaScript', points: 8 }] }] } } };
