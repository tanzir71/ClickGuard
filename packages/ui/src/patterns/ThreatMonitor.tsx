...
function VisitorPanel({ visitor, protectionPaused, onClose, onPrevious, onNext, onOpenJourney, onAction, confirmAction, onConfirm, selectedVisitId, onSelectVisit }: { visitor: VisitorVM; protectionPaused: boolean; onClose: () => void; onPrevious: () => void; onNext: () => void; onOpenJourney: (visitId?: string) => void; onAction: (action: 'allow' | 'block' | null) => void; confirmAction: 'allow' | 'block' | null; onConfirm: () => void; selectedVisitId: string; onSelectVisit: (id: string) => void }) {
  ...
  const hasRelated = related && [related.sameFingerprintIps, related.subnet24Ips, related.asnVisitorCount, related.networkBlockedAccounts30d].some((count) => count > 0);
  ...
  const interactionLabel = visitor.typicalInteraction === 'none' ? `None on ${noneVisits} of ${visitor.visits.length} visits` : `${visitor.typicalInteraction[0].toUpperCase() + visitor.typicalInteraction.slice(1)} on most visits`;
  return <aside className={styles.visitorPanel} role="complementary" aria-label={`Visitor ${visitor.ip} details`}><header className={styles.panelHeader}><div><span className={styles.eyebrow}>Visitor</span><h2 className={styles.mono}>{visitor.ip} <button type="button" aria-label="Copy IP" onClick={() => navigator.clipboard?.writeText(visitor.ip)}><Copy /></button></h2><p>{[visitor.city, visitor.region, visitor.country].filter(Boolean).join(', ') || 'Location unknown'}</p><small>{[visitor.isp, visitor.asn, visitor.networkType, `${visitor.deviceCount} ${visitor.deviceCount === 1 ? 'device' : 'devices'}`].filter(Boolean).join(' · ')}</small></div><Stack direction="row" gap="1"><IconButton label="Previous visitor" onClick={onPrevious}>↑</IconButton><IconButton label="Next visitor" onClick={onNext}>↓</IconButton><IconButton label="Close details" onClick={onClose}><X /></IconButton></Stack><VerdictCard status={visitor.status} sentence={visitor.verdict} meta={`${visitor.decisionBy ?? 'Auto'} · ${visitor.riskScore}`} /></header><div className={styles.panelBody}>
    <PanelSection label={visitor.status === 'monitoring' ? "Will be excluded on" : protectionPaused ? "Exclusions · enforcement paused" : "Excluded on"}><ExclusionList rows={visitor.exclusions} protectionPaused={protectionPaused} /></PanelSection>
    <PanelSection label="How it unfolded"><DecisionRoute visits={visitor.visits} blockedAtVisitId={visitor.blockedAtVisitId} exclusions={visitor.exclusions} allowedBy={visitor.allowedBy} threshold={visitor.threshold} status={visitor.status} /></PanelSection>
    ...
    <PanelSection label={visitor.status === 'clean' ? 'Why it looks real' : visitor.status === 'monitoring' ? 'Why it’s not blocked yet' : visitor.status === 'allowed' ? 'What we’d have blocked on' : 'Why we blocked'}>{visitor.topSignals.filter((signal) => !['score_limit', 'decay'].includes(signal.id)).slice(0, 5).map((signal) => <SignalBar key={`${signal.id}-${signal.points}`} signal={signal} />)}</PanelSection>
    <PanelSection label="Signals at a glance"><div className={styles.signalGrid}><SignalMeter label="Location" value={`${visitor.city ?? 'Unknown'}, ${visitor.country ?? '—'}`} level={visitor.country ? 4 : 0} /><SignalMeter label="Interaction" value={interactionLabel} level={{ none: 0, low: 1, medium: 2, high: 4 }[visitor.typicalInteraction] ?? 0} tone={visitor.typicalInteraction === 'none' ? 'warning' : 'neutral'} /><SignalMeter label="Bot probability" value={`${Math.round(visitor.maxBotProbability * 100)}% max · ${Math.round(visitor.avgBotProbability * 100)}% avg`} level={Math.ceil(visitor.maxBotProbability * 4)} tone={visitor.maxBotProbability >= .7 ? 'danger' : visitor.maxBotProbability >= .5 ? 'warning' : 'neutral'} /><SignalMeter label="VPN / proxy" value={vpnVisits ? `Yes · ${vpnVisits} of ${visitor.visits.length} visits` : 'No'} level={visitor.vpnProxyAny ? 4 : 0} tone={visitor.vpnProxyAny ? 'warning' : 'neutral'} /><SignalMeter label="Form fill" value={visitor.formFills.invalid ? `${visitor.formFills.invalid} invalid` : visitor.formFills.valid ? `${visitor.formFills.valid} valid` : 'Never submitted'} level={visitor.formFills.invalid ? 4 : visitor.formFills.valid ? 2 : 0} tone={visitor.formFills.invalid ? 'danger' : visitor.formFills.valid ? 'success' : 'neutral'} /><SignalMeter label="Conversion" value={visitor.conversions ? `${visitor.conversions} · $${visitor.conversionValueTotal.toFixed(0)}` : 'None'} level={visitor.conversions ? 4 : 0} tone={visitor.conversions ? 'success' : 'neutral'} /></div></PanelSection>
    <PanelSection label="Spend"><SpendReceipt visitor={visitor} /></PanelSection>
    {hasRelated && related && <PanelSection label="Related"><dl className={styles.related}>
      {related.sameFingerprintIps > 0 && <KeyValue label="Same device on other IPs" value={related.sameFingerprintIps} />}
      {related.subnet24Ips > 0 && <KeyValue label="Same /24 subnet" value={`${related.subnet24Ips} ${related.subnet24Ips === 1 ? 'visitor' : 'visitors'}`} />}
      {related.asnVisitorCount > 0 && <KeyValue label="Same network" value={`${related.asnVisitorCount} visitors${related.asnBlockedCount > 0 ? ` · ${related.asnBlockedCount} blocked` : ''}`} />}
      {related.networkBlockedAccounts30d > 0 && <KeyValue label="Network intelligence" value={`${related.networkBlockedAccounts30d} other ClickGuard accounts`} />}
    </dl></PanelSection>}
