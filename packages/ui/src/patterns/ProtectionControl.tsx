...
export function PauseProtectionDialog({
  onCancel,
  onConfirm,
}: {
  onCancel: () => void;
  ...
  return (
    <dialog
      ...
      onClick={(event) => {
        if (event.target === event.currentTarget) closeThen(onCancel);
      }}
      onKeyDown={(event) => {
      ...
      <div className={styles.dialogBody}>
        ...
        <div id={descriptionId}>
          ...
          <div className={styles.consequences}>
            ...
            <p>
              <strong>Risky clicks will no longer be blocked.</strong>
              <br />
              Monitoring continues, but wasted spend can keep growing while protection is paused.
            </p>
          </div>
          <p className={styles.disclosure}>
            Prototype simulation — no ad accounts are changed. Resume protection at any time.
          </p>
        </div>
        <div className={styles.dialogActions}>
          ...
          <Button variant="danger" onClick={() => closeThen(onConfirm)}>
...
export function ProtectionPausedBanner({ onResume }: { onResume: () => void }) {
  return (
    <section className={styles.banner} aria-label="Protection warning">
      <ShieldOff aria-hidden="true" />
      <div role="status">
        <strong>All protection paused</strong>
        <p>Monitoring continues. New and existing blocks are suspended in this demo.</p>
      </div>
      <Button size="sm" variant="primary" iconStart={<ShieldCheck aria-hidden="true" />} onClick={onResume}>
        Resume protection
      </Button>
    </section>
  );
