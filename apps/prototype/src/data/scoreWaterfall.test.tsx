...
afterEach(cleanup);
const hit = (signalId: SignalId, points: number): SignalHit => ({
  signalId,
  points,
  value: 'Observed in test',
  severity: 'med',
});
...
describe('per-visit score arithmetic', () => {
  it('H1–H13 retain their scenario statuses and specified final-score tolerances', () => {
    ...
    for (const visitor of heroes)
      if (visitor.scenario! in targets)
        expect(
          Math.abs(visitor.riskScore - targets[visitor.scenario!]),
          visitor.scenario,
    ...
    for (const visitor of rawData.visitors) {
      ...
      if (['blocked', 'pending', 'failed'].includes(visitor.status)) {
        ...
        expect(
          visits
            .slice(0, visits.indexOf(decision))
            .every((visit) => visit.source !== 'paid' || visit.scoreAfter < visitor.threshold),
          visitor.ip,
        ).toBe(true);
      }
      if (visitor.status === 'monitoring' && visits.some((visit) => visit.source === 'paid'))
        expect(Math.max(...visits.map((visit) => visit.scoreAfter)), visitor.ip).toBeLessThan(
          visitor.threshold,
        );
    }
  });
  ...
  it('H1–H13 and every crowd visit reconcile exactly, including limits and decay', () => {
    for (const visit of rawData.visits) {
      expect(visit.scoreBefore + visit.scoreSteps.reduce((sum, step) => sum + step.points, 0), visit.id).toBe(
        visit.scoreAfter,
      );
  ...
  it('H1 visit 6 uses only the remaining category capacity and explicitly caps the total', () => {
    ...
    for (const visit of visits)
      for (const step of visit.scoreSteps) {
        ...
        expect(Math.abs(step.points), `${visit.id}: ${step.signalId}`).toBeLessThanOrEqual(
          SIGNAL_CAPS[step.signalId as SignalId] - before,
        );
  ...
  it('H4 purchase records a negative conversion step, not an unexplained rising score', () => {
    ...
    expect(converted.scoreSteps).toContainEqual(
      expect.objectContaining({ signalId: 'converted', points: -25 }),
    );
  ...
  it('H10 lower-bound clamping and seven-day inactivity adjustments remain visible', () => {
    ...
    expect(floor.scoreSteps.at(-1)).toMatchObject({
      label: 'Score limit',
      points: 25,
      reason: 'floored at 0',
    });
    ...
    expect(decay.scoreSteps).toEqual([
      { signalId: 'decay', label: 'Inactivity decay', reason: '14 inactive days', points: -22 },
    ]);
  });
  ...
  it('H1 counts once-only tiers and additive caps independently', () => {
    ...
    expect(applyVisitScore(30, [hit('interaction_none', 6)], counted).scoreSteps[0]).toMatchObject({
      points: 2,
      rawPoints: 6,
    });
  });
  ...
  it('H1–H13 final scores and cumulative signal totals share the same ledger', () => {
    for (const visitor of visitorViewModels) {
      ...
      expect(
        visitor.topSignals.reduce((sum, signal) => sum + signal.points, 0),
        visitor.ip,
      ).toBe(visitor.riskScore);
      for (let index = 1; index < visitor.visits.length; index++)
        expect(visitor.visits[index].scoreBefore).toBe(visitor.visits[index - 1].scoreAfter);
    }
  });
  ...
  it('H3 unpaid returns only rise when another raising signal actually fires', () => {
    for (const visit of rawData.visits.filter((item) => item.source !== 'paid')) {
      if (
        visit.scoreSteps.some((step) => step.signalId === 'returning_organic') &&
        visit.scoreAfter > visit.scoreBefore
      ) {
        expect(
          visit.scoreSteps.some((step) => step.points > 0 && step.signalId !== 'score_limit'),
          visit.id,
        ).toBe(true);
      }
    }
  ...
  it('H1 scoring is deterministic and ignores hand-authored scores', () => {
    const input = {
      ...rawData,
      visitors: rawData.visitors.map((visitor) => ({ ...visitor, riskScore: 999 })),
      visits: rawData.visits.map((visit) => ({ ...visit, scoreBefore: 999, scoreAfter: 999 })),
    };
    const original = JSON.stringify(input);
    const scored = scoreDataSet(input);
    expect(JSON.stringify(input)).toBe(original);
    expect(scored.visits.map((visit) => visit.scoreSteps)).toEqual(
      scoreDataSet(input).visits.map((visit) => visit.scoreSteps),
    );
  ...
  it('H1 separates This visit arithmetic from the Whole journey signal table', () => {
    ...
    expect(screen.getByText('Score after this visit').nextElementSibling?.textContent).toContain(
      '100 ◆ crossed 70',
    );
    ...
    expect(
      screen.getByText('Signals are capped by category. Scores decay after seven inactive days.'),
    ).toBeTruthy();
  });
});
