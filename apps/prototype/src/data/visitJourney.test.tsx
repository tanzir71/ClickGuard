...
describe('table risk journeys', () => {
  ...
  it('ends each right-angle step at its unchanged visit marker', () => {
    const chart = buildVisitJourney(sample([10, 30, 20]), 70, 'test_1');
    expect(chart.points.map(({ x, y }) => [x, y])).toEqual([[8, 33.8], [91, 27.4], [174, 30.6]]);
    expect(chart.beforePath).toBe('M 8 37 V 33.8 H 91 V 33.8 V 27.4');
    expect(chart.afterPath).toBe('M 91 27.4 H 174 V 27.4 V 30.6');
