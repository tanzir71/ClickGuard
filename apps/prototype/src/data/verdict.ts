...
const paidWindow = (visits: Visit[]) => {
  ...
  const minutes = Math.max(
    1,
    Math.round((Date.parse(paid.at(-1)!.startedAt) - Date.parse(paid[0].startedAt)) / 60000),
  );
  const span =
    minutes < 60
      ? count(minutes, 'minute')
      : minutes < 1440
        ? count(Math.ceil(minutes / 60), 'hour')
        : count(Math.ceil(minutes / 1440), 'day');
...
export const SIGNAL_PHRASES: Record<SignalHit['signalId'], (ctx: PhraseContext) => string> = {
  bot_probability: ({ visits }) =>
    `${Math.round(Math.max(...visits.map((visit) => visit.botProbability)) * 100)}% likely automated`,
  ...
  vpn_proxy: ({ visits }) =>
    visits.filter((visit) => visit.vpnProxy).length === 1 ? 'a VPN visit' : 'visits through a VPN or proxy',
  form_invalid_email: ({ visits }) =>
    count(
      visits.filter((visit) => ['invalid', 'disposable'].includes(visit.formFill?.deliverability ?? ''))
        .length,
      'form fill with an undeliverable email',
      'form fills with undeliverable emails',
    ),
  location_outside_targeting: () => 'visits outside campaign targeting',
  timezone_mismatch: ({ visits }) =>
    `a browser timezone (${visits.at(-1)?.device.timezone}) that differs from the IP location`,
  ...
  converted: ({ visits }) =>
    visits.some((visit) => visit.conversionType === 'purchase')
      ? 'a purchase was completed'
      : 'a conversion was recorded',
  form_valid_email: () => 'a form was submitted with a deliverable email',
  interaction_high: ({ visits }) =>
    `real engagement on ${count(visits.filter((visit) => visit.interaction.level === 'high').length, 'visit')}`,
  ...
  network_intel: ({ visitor }) =>
    `this network was blocked on ${count(visitor.networkBlockedAccounts30d, 'other account')}`,
};
...
export function verdict(visitor: Visitor, visits: Visit[], topSignals: SignalHit[], timeZone: string) {
  ...
  const date = (at: string) =>
    new Date(at).toLocaleDateString('en-US', { month: 'short', day: 'numeric', timeZone });
  const time = (at: string) =>
    new Date(at).toLocaleTimeString('en-US', { hour: 'numeric', minute: '2-digit', timeZone });
  ...
  if (visitor.networkType === 'datacenter' && !raising.some((signal) => signal.signalId === 'datacenter_ip'))
    reason += ' from a data-center IP';
  ...
  if (visitor.status === 'blocked')
    return (
      `Blocked on ${date(visitor.blockedAt!)} at ${time(visitor.blockedAt!)} ` +
      `after visit ${triggerIndex + 1} of ${visits.length}: ${reason}.`
    );
  if (visitor.status === 'pending')
    return (
      `Blocking now: The score crossed ${visitor.threshold} at visit ${triggerIndex + 1}, ` +
      'and exclusions are syncing to connected ad platforms.'
    );
  if (visitor.status === 'failed') {
    const failed = visitor.exclusions.find((row) => row.state === 'failed');
    const platform =
      failed?.platform === 'google_ads'
        ? 'Google Ads'
        : failed?.platform === 'meta_ads'
          ? 'Meta Ads'
          : 'An ad platform';
    return (
      `We made a block decision at visit ${triggerIndex + 1}, but ${platform} rejected the exclusion. ` +
      (failed?.error
        ? sentenceCase(failed.error.replace(/[.!]+$/, ''))
        : 'Review the exclusion error to stop more paid clicks') +
      '.'
    );
  }
  if (visitor.status === 'allowed')
    return (
      `Always allowed by ${visitor.allowedBy?.user ?? 'your team'}` +
      (visitor.allowedBy ? ` on ${date(visitor.allowedBy.at)}` : '') +
      '. ' +
      (triggerIndex >= 0 ? `A prior block decision was recorded at visit ${triggerIndex + 1}. ` : '') +
      (visitor.allowedBy?.note
        ? `Note: “${visitor.allowedBy.note}”.`
        : 'The manual allowance takes priority.')
    );
  if (visitor.status === 'monitoring' && !visits.some((visit) => visit.source === 'paid'))
    return 'Suspicious, but there’s nothing to block: This visitor has never clicked your ads, so no ad spend is at risk.';
  if (visitor.status === 'monitoring')
    return (
      `Not blocked. Risk is ${visitor.riskScore}, ` +
      `${count(Math.max(0, visitor.threshold - visitor.riskScore), 'point')} under the block threshold: ${reason}.`
    );
  return (
    `Looks like a real visitor: ${count(visits.length, 'visit')} with normal engagement` +
    (visits.some((visit) => visit.converted) ? ' and a conversion' : '') +
    '.'
  );
}
