...
export interface Visit {
  ...
  conversionType?:
    'purchase' | 'add_to_cart' | 'begin_checkout' | 'submit_form' | 'sign_up' | 'request_quote';
  ...
  converted?: boolean;
  riskObservations?: {
    outsideTargeting?: boolean;
    timezoneMismatch?: boolean;
    businessHoursPattern?: boolean;
    deviceSpoofing?: boolean;
    rotatingFingerprints?: boolean;
  };
}
...
export interface Visitor {
  ...
  asnName?: string;
  reverseDns?: string;
  abuseListHits?: string[];
  decision?: { by: 'auto' | { user: string }; at: string; rule: string };
  reviewed: boolean;
  related: {
    sameFingerprintIps: string[];
    subnet24Ips: string[];
...
export interface DataSet {
  account: { connectedPlatforms: Platform[]; timeZone: string };
  visitors: Visitor[];
  visits: Visit[];
}
